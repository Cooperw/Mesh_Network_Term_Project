"""
This is a modified version of the library found at https://pypi.org/project/rpi-rf/

We made three significant changes:
	-Longer packet sending/receiving
	-Raw binary code input
	-RF protocol cleanup

Our comments can be identified with (JCL)
...
# We changed this value to allow for longer packets (JCL)
MAX_CHANGES = 205
...
class RFDevice:
    ...
    # Modified the tx_length to be our max packet size (JCL)
    def __init__(self, gpio,
                 tx_proto=1, tx_pulselength=None, tx_repeat=10, tx_length=100, rx_tolerance=80):
    ...
    def tx_bin(self, rawcode):
        ...
        for _ in range(0, self.tx_repeat):
            for byte in range(0, len(rawcode)):
                if rawcode[byte] == '0':
                    if not self.tx_l0():
                        return False
                else:
    ...
    def _rx_waveform(self, pnum, change_count, timestamp):
        ...
        if self._rx_change_count > 6 and code != 0:
            self.rx_code = str("{0:b}".format(code))[1:]
